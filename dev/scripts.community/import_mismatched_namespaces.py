...
def do_check():
    ...
    for idr,row in enumerate(rows):
        ...
        legacy_sync_from_upstream(github_user=row['provider_namespace__name'], role_name=row['role_name'])
        legacy_sync_from_upstream(github_user=row['namespace_name'], role_name=row['role_name'])
